...
class RaportsController extends Controller {
	...
	{
		$getProgramStudyId = School::where('applicant_id', $applicant_id)->pluck('program_study_id');
	 ...
	 * @return Response
	 */
	public function update($applicant_id, RaportRequest $request)
	{
		//dd($request->all());
		$raport = Raport::findOrFail($applicant_id);
		...
		return redirect('applications/'.$raport->applicant_id);
	}
